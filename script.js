...
class AnimationController {
  ...
  observeElements() {
    ...
    // Do not force opacity/transform here because carousel slides are outside viewport (overflow hidden)
  ...
  animateCounter(counter) {
    ...
    if (current < target) {
      ...
      counter.textContent =
        Math.floor(newValue) + (counter.textContent.includes("%") ? "%" : "");
...
class ProjectsCarousel {
  ...
  previous() {
    this.currentIndex =
      (this.currentIndex - 1 + this.projectCards.length) %
      this.projectCards.length;
    this.updateCarousel();
  }

  updateCarousel() {
    // Compute robust pixel-based offset using actual positions (accounts for gap and responsive widths)
    if (this.carousel && this.projectCards.length) {
      let step = 0;
      if (this.projectCards.length > 1) {
        const firstRect = this.projectCards[0].getBoundingClientRect();
        const secondRect = this.projectCards[1].getBoundingClientRect();
        step = Math.round(
          secondRect.width + (secondRect.left - firstRect.right),
        );
      }
      if (!step) {
        const firstCard = this.projectCards[0];
        const cardWidth = firstCard.offsetWidth;
        const styles = getComputedStyle(this.carousel);
        const gapValue =
          styles.gap || styles.columnGap || styles.rowGap || "0px";
        const gapPx = parseFloat(gapValue) || 0;
        step = Math.round(cardWidth + gapPx);
      }
      const offset = this.currentIndex * step;
      this.carousel.style.transform = `translateX(-${offset}px)`;
    }

    // Update active state for visual emphasis
...
class MicroInteractions {
  ...
  initCardHoverEffects() {
    const cards = document.querySelectorAll(
      ".skill-card, .project-card, .stat-item, .timeline-content",
    );
    cards.forEach((card) => {
  ...
  initSmoothTransitions() {
    ...
    const elements = document.querySelectorAll(
      ".skill-card, .project-card, .stat-item, .timeline-content, .contact-form",
    );
...
// Simple Gallery Rotator for College Section
class CollegeGalleries {
  constructor() {
    this.galleries = document.querySelectorAll(".college-gallery");
    this.timers = [];
    this.init();
  }

  init() {
    this.galleries.forEach((gallery) => this.startRotation(gallery));
  }

  startRotation(gallery) {
    ...
    // 2 seconds interval per requirement
    const timer = setInterval(rotate, 2000);
    this.timers.push(timer);

    // Pause on hover
    gallery.addEventListener("mouseenter", () => clearInterval(timer));
    gallery.addEventListener("mouseleave", () => {
      const newTimer = setInterval(rotate, 2000);
...
class ContactForm {
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      const mailto = `mailto:abishsabi31@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
